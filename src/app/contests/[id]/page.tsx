...
export default function ContestArenaPage() {
  ...
  const handleSelectQuestion = (question: any) => {
    ...
    // Optionally reset editor state when changing questions
    setCode("");
    setInput("");
    setOutput("");
    setStatus(null);
  ...
  if (loading) return <div style={{ color: 'white', textAlign: 'center', paddingTop: '50px' }}>Loading Contest...</div>;
  if (error) return <div style={{ color: 'red', textAlign: 'center', paddingTop: '50px' }}>Error: {error}</div>;
  if (!contest) return <div style={{ color: 'white', textAlign: 'center', paddingTop: '50px' }}>Contest not found.</div>;
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#111', color: '#fff', padding: '20px' }}>
      ...
      <div style={{ flex: '0 0 300px', background: '#181818', borderRadius: '12px', padding: '24px', marginRight: '20px', overflowY: 'auto' }}>
        <h2 style={{ fontSize: '24px', fontWeight: 700, color: '#61dafb', marginBottom: '24px' }}>{contest.name}</h2>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {selectedQuestion ? (
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#181818', borderRadius: '12px' }}>
            <p style={{ fontSize: '18px', color: '#aaa' }}>Select a question to start.</p>
          </div>
        )}
      </div>
    </div>
